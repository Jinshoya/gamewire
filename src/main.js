...
function startCountdowns() {
  setInterval(() => {
    ...
    document.querySelectorAll('.countdown').forEach(el => {
      ...
      // IMPORTANT CHANGE: Append '+0300' to the date string to explicitly specify UTC+5 offset
      // This forces Date.parse to interpret the string as UTC+5, regardless of user's local timezone.
      const start = new Date(startDateTimeStr + ' +0300');
      const startUtcTime = start.getTime(); // Get UTC milliseconds for consistent calculations

      const end = endDateTimeStr ? new Date(endDateTimeStr + ' +0300') : null;
...
parsedEvents.sort((a, b) => {
  const dateA = new Date(a.date + ' +0300');
  const dateB = new Date(b.date + ' +0300');
...
const dateForLocalDisplay = new Date(data.date + ' +0300');
const endForLocalDisplay = data.end ? new Date(data.end + ' +0300') : null;
...
? formatRange(data.date + ' +0300', data.end + ' +0300')
: formatSingle(data.date + ' +0300');

const utcDateStr = isSteam
? formatRangeUTC(data.date + ' +0300', data.end + ' +0300')
: formatSingleUTC(data.date + ' +0300');
