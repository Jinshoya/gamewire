...
for (let i = 0; event_date_divs.length > i; i++) {
    // Fixed: Display event time in Europe/Riga timezone
    let eventDateUTC = new Date(Date.parse(event_date_divs[i].getAttribute("data-start-event-date")));
    event_date_divs[i].innerHTML = eventDateUTC.toLocaleString(navigator.language, {
...
for (let i = 0; event_date_divs.length > i; i++) {
    let timer_id = setInterval(function () {
        // Fixed: Parse event date as UTC
        let event_date = new Date(event_date_divs[i].getAttribute("data-start-event-date") + 'Z').getTime();
